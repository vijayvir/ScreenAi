...
public class ReactiveScreenShareHandler implements WebSocketHandler {
    ...
    public Mono<Void> handle(WebSocketSession session) {
        ...
        // Authenticate via Authorization header (optional — guests allowed without
        // token)
        return authHandler.authenticate(session.getHandshakeInfo().getHeaders(), sessionId, ipAddress)
                .switchIfEmpty(Mono.defer(() -> {
                    // Invalid token provided — reject the connection
                    logger.warn("🚫 Invalid token rejected: {} from IP: {}", sessionId, ipAddress);
                    auditService.logConnectionBlocked(ipAddress, "invalid_token").subscribe();
                    sendErrorAndClose(session, ErrorCode.AUTH_001, "Invalid authentication token.");
                    return Mono.empty();
                }))
                .flatMap(user -> {
                ...
                if (viewerInfoOpt != null && viewerInfoOpt.isPresent()) {
                    ...
                    try {
                        ...
                        if (viewerSession.isOpen()) {
                            WebSocketMessage binaryMsg = viewerSession
                                    .binaryMessage(factory -> factory.wrap(videoData));
    ...
    private void handleJoinRoom(WebSocketSession session, JsonNode json) {
        ...
        if (room.isPasswordProtected()) {
            ...
            if (!accessGranted) {
                ...
                auditService.logRoomAccessDenied(username, sessionId, roomId, ipAddress, "Invalid credentials")
                        .subscribe();
                return;
            }
        }
    ...
    private void addViewerToRoom(ReactiveRoom room, String sessionId, String username,
        ...
        if (initSegment != null && initSegment.length > 0) {
            try {
                WebSocketMessage binaryMsg = session.binaryMessage(factory -> factory.wrap(initSegment));
                if (sink != null) {
        ...
        if (sink != null && room.getPresenterSession() != null) {
            try {
                String message = String.format(
                        ...
                        viewerSessionId, viewerUsername != null ? viewerUsername : "anonymous",
                        room.getPendingViewerCount());
    ...
    private String createWelcomeMessage(String sessionId, AuthenticatedUser user) {
        String username = user != null ? user.username() : "anonymous";
        String userRole = user != null ? user.role() : "GUEST";
        boolean isGuest = "GUEST".equals(userRole);
        return String.format(
                "{\"type\":\"connected\",\"sessionId\":\"%s\",\"username\":\"%s\",\"userRole\":\"%s\",\"isGuest\":%s,\"message\":\"Connected to ScreenAI Relay Server (WebFlux+Netty)\",\"role\":\"pending\"}",
                sessionId, username, userRole, isGuest);
    }
